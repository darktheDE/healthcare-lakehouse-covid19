...
def build_spark() -> SparkSession:
    return (
        SparkSession.builder.appName("silver-cleansing")
...
def build_covid_clinical_master(
    ...
    select_columns = [
        F.col("p.patient_id"),
        F.col("p.birthdate").alias("patient_birthdate"),
        F.col("p.deathdate").alias("patient_deathdate"),
        F.col("p.ssn"),
        F.col("p.drivers"),
        F.col("p.passport"),
        F.col("p.prefix"),
        ...
        F.coalesce(F.col("e.total_claim_cost_sum"), F.lit(0).cast("decimal(18,2)")).alias(
            "total_claim_cost_sum"
        ),
...
def print_metadata_checks(spark: SparkSession, table_name: str) -> None:
    print("\n[CHECK] Hive Metastore metadata for target table:")
    print(f"  - table exists: {spark.catalog.tableExists(table_name)}")

    tables_df = spark.sql(f"SHOW TABLES IN {SILVER_NAMESPACE}")
    tables_df.show(truncate=False)

    metadata_df = spark.sql(f"DESCRIBE EXTENDED {table_name}")
    selected_keys = {"Type", "Provider", "Location", "Owner", "Table Properties"}
    metadata_df.filter(F.col("col_name").isin(*selected_keys)).show(truncate=False)
...
def main() -> None:
    ...
    print("=" * 72)
    print("  Bronze -> Silver COVID clinical master job")
    print("=" * 72)
