...
Thực hiện lần lượt theo các bước sau để đảm bảo chương trình không bị lỗi thứ tự thực thi của container

### Bước 1: Chạy các dịch vụ nền tảng
...
```bash
docker compose --env-file .env -f deploy/docker-compose.yml up -d minio postgres-source metastore-db hive-metastore spark-iceberg
```

### Bước 2: Nạp dữ liệu từ CSDL OLTP vào Data Lake trong raw/ của MinIO

```bash
docker compose --env-file .env -f deploy/docker-compose.yml up spark-raw-job
```

### Bước 3: Chạy Bronze job

```bash
docker compose --env-file .env -f deploy/docker-compose.yml up spark-bronze-job
```

### Bước 4: Chạy Silver job

```bash
docker compose --env-file .env -f deploy/docker-compose.yml up spark-silver-job
```

### Bước 5: Verify qua Spark log

Các tín hiệu mong đợi trong log:

- `[INFO] COVID clinical master rows: 88166`
- `[INFO] Writing target table: hospital.silver.covid_clinical_master`
- `[INFO] Persisted row count: 88166`
- `[DONE] Silver table enrichment completed successfully`

### Bước 6: Verify bằng Trino
...
### Bước 7: Verify vật lý trên MinIO (Iceberg files)
